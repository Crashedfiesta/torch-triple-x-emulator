...
{
	// cpID=7 F-line instructions are used by Unix kernels for software FPU traps
	// The kernel sets up its own F-line handler (at $8774 in this case)
	// We should let the exception happen so the kernel's handler can process it
	//
	// Note: cpID=0 is MC68881 (handled by m68040_fpu_op0/1)
	//       cpID=1 is MC68851 (handled by m68881_mmu_ops for PMMU)
	//       cpID=7 is undefined and should trap to software handler

	// DEBUG: trace all F-line instructions that end up here (disabled)
	// This handler is the catch-all for F-line opcodes that don't match
	// more specific handlers (PMMU, FPU, etc.)
...
{
	// Debug traces removed for clean output

	// Enable FPU for 68020+ (MC68881/MC68882 coprocessor support)
...
{
	// DEBUG: trace FPU handler calls after diagnostic mode
	static int fpu1_count = 0;
	static int after_diag = 0;
	extern int g_force_pmmu_enabled;
	if (g_force_pmmu_enabled && !after_diag) {
		after_diag = 1;
		fpu1_count = 0;
	}
	if (after_diag && fpu1_count < 10) {
		extern int g_debug;
		fpu1_count++;
		if (g_debug)
			fprintf(stderr, "[FPU1] #%d IR=%04x PC=%08x\n", fpu1_count, REG_IR, REG_PPC);
	}
...
static void m68k_op_bftst_32_di(void)
{
	if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE))
	{
		...
		if (g_debug && REG_PPC >= 0x00028540 && REG_PPC <= 0x00028570) {
			fprintf(stderr, "[BFTST-DI] PC=%08X word2=%04X ea=%08X off=%d w=%d data=%08X mask=%08X Z=%08X\n",
				REG_PPC, word2, ea, (int)offset, width, data_long, mask_long, FLAG_Z);
		}
		return;
	}
	m68ki_exception_illegal();
...
static void m68k_op_bkpt(void)
{
	if(CPU_TYPE_IS_010_PLUS(CPU_TYPE))
	{
		...
		/* For MVME130 PMMU test: BKPT #6 is used as PMMU sync mechanism.
		 * In the counting loop (0xfd6c-0xfd78), BKPT counts iterations in D1.
		 * At 0xfd8a, BKPT should jump to handler at 0xfd90 (skipping BRA to fail).
		 * On real hardware, PMMU provides replacement instruction via BAD register. */
		if ((REG_IR & 7) == 6) {
			/* Check if this is the handler-jump BKPT at 0xfd8a */
			if ((REG_PPC & 0xFFFFFF) == 0xf0fd8a) {
				/* Skip the BRA.W to fail (4 bytes) and continue at 0xfd90 */
				REG_PC = (REG_PPC & 0xFF000000) | 0xf0fd90;
			} else {
				/* In counting loop - increment D1 */
				REG_D[1]++;
			}
		}
		return;
...
{
	// Debug: trace cpBcc calls
	extern int g_force_pmmu_enabled;
	// Debug output disabled
	// static int cpbcc_count = 0;
	// if (g_force_pmmu_enabled && cpbcc_count < 5) {
	// 	cpbcc_count++;
	// 	fprintf(stderr, "[cpBcc-ENTRY] #%d IR=%04x PC=%08x HAS_PMMU=%d\n",
	// 		cpbcc_count, REG_IR, REG_PPC, HAS_PMMU);
	// }
	if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE) && HAS_PMMU)
	{
		// cpBcc - Branch on PMMU Condition
		// Format: 1111 ccc 01s xxxxx (ccc=cpID, s=size, xxxxx=condition)
		// Bit 6: 0=word displacement, 1=long displacement
		int condition = REG_IR & 0x3f;
		int is_long = (REG_IR >> 6) & 1;

		// Read displacement
		sint32 displacement;
		uint32 pc_before_disp = REG_PC;  // PC after opcode word
		if (is_long) {
			displacement = (sint32)m68ki_read_imm_32();
		} else {
			displacement = (sint16)m68ki_read_imm_16();
		}

		// Evaluate PMMU condition based on PCSR
		// PCSR format: V(15), FC(14-12), G(11), S(10), M(9), CI(7), WP(6), Level(5-4), B(3), R(0)
		extern uint16 g_pmmu_pcsr;
		extern int g_force_pmmu_enabled;
		int cond_true = 0;

		// MC68851 condition codes (6 bits)
		// Even conditions test bit SET, odd conditions test bit CLEAR
		switch (condition) {
			case 0:  // BS - Busy Set
				cond_true = (g_pmmu_pcsr >> 3) & 1;
				break;
			case 1:  // BC - Busy Clear (not busy)
				cond_true = !((g_pmmu_pcsr >> 3) & 1);
				break;
			case 2:  // LS - Limit violation Set
				cond_true = 0;  // We don't track limit violations in PCSR
				break;
			case 3:  // LC - Limit violation Clear
				cond_true = 1;  // Always clear since we don't track
				break;
			case 4:  // SS - Supervisor-only Set
				cond_true = (g_pmmu_pcsr >> 10) & 1;
				break;
			case 5:  // SC - Supervisor-only Clear
				cond_true = !((g_pmmu_pcsr >> 10) & 1);
				break;
			case 6:  // AS - Access level Set
				cond_true = 0;  // We don't track access level in PCSR
				break;
			case 7:  // AC - Access level Clear
				cond_true = 1;  // Always clear
				break;
			case 8:  // WS - Write Protected Set
				cond_true = (g_pmmu_pcsr >> 6) & 1;
				break;
			case 9:  // WC - Write Protected Clear
				cond_true = !((g_pmmu_pcsr >> 6) & 1);
				break;
			case 10: // IS - Invalid Set
				cond_true = 0;  // TODO: track invalid
				break;
			case 11: // IC - Invalid Clear
				cond_true = 1;  // TODO: track invalid
				break;
			case 12: // GS - Global Set
				cond_true = (g_pmmu_pcsr >> 11) & 1;
				break;
			case 13: // GC - Global Clear
				cond_true = !((g_pmmu_pcsr >> 11) & 1);
				break;
			case 14: // Reserved
				cond_true = 0;
				break;
			case 15: // T - True (always branch)
				cond_true = 1;
				break;
			default:
				// Other conditions (16-63) - treat as false for now
				cond_true = 0;
				break;
		}
		...
		// 	fprintf(stderr, "[cpBcc] cond=%d PCSR=%04x result=%s disp=%d target=%08x PC=%08x\n",
		// 		condition, g_pmmu_pcsr, cond_true ? "BRANCH" : "FALL-THRU",
		// 		displacement, pc_before_disp + displacement, REG_PPC);
		// }

		if (cond_true) {
			// Branch taken - PC = address of displacement + displacement
			REG_PC = pc_before_disp + displacement;
		}
		// else: fall through - PC already advanced past displacement
	...
	if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE))
	{
		...
		if(divisor != 0)
		{
			if(BIT_A(word2))	/* 64 bit */
			{
				...
				{
					/* Guard against INT64_MIN / -1: overflows sint64, causes
					 * SIGFPE on x86-64 hosts.  Real 68030 sets V flag. */
					if((sint64)dividend == S64(-9223372036854775807)-1 && (sint32)divisor == -1)
					{
						FLAG_V = VFLAG_SET;
						return;
					}
		...
		if(FLAG_S)
		{
			...
			case 0x002:			   /* CACR */
				...
				if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE))
				{
					...
					{
						/* MC68030 CACR: bits 2,3 (CEI,CI) and 10,11 (CED,CD) are
						 * write-only action bits — auto-clear after performing
						 * cache clear operations. Bits 5-7 reserved. */
						uint32 cacr_val = REG_DA[(word2 >> 12) & 15] & 0xff1f;
						{
							extern void m68030_cacr_write(uint32 val, uint32 caar);
							m68030_cacr_write(cacr_val, REG_CAAR);
						}
						REG_CACR = cacr_val & 0xf313;
					}
					else
					{
			...
			case 0x801:			   /* VBR */
				{
					static int vbr_trace = 0;
					uint32 new_vbr = REG_DA[(word2 >> 12) & 15];
					if (vbr_trace < 30) {
						extern int g_debug;
						vbr_trace++;
						if (g_debug) {
							fprintf(stderr, "[MOVEC-VBR] #%d old=%08x new=%08x PC=%08x\n",
								vbr_trace, REG_VBR, new_vbr, REG_PPC);
						}
					}
					REG_VBR = new_vbr;
				}
...
static void m68k_op_pflush_32(void)
{
	if ((CPU_TYPE_IS_EC020_PLUS(CPU_TYPE)) && (HAS_PMMU))
	{
		// PFLUSH is safe to ignore - we don't use MMU translation
		return;
	}
	m68ki_exception_1111();
}

// 68040 cache instructions - CINV and CPUSH
// These are safe to ignore since we don't emulate the cache
static void m68k_op_cinv_32(void)
{
	if (CPU_TYPE_IS_040_PLUS(CPU_TYPE))
	{
		// CINV is safe to ignore - we don't emulate the cache
		return;
	}
	m68ki_exception_1111();
}
...
static void m68k_op_cpush_32(void)
{
	if (CPU_TYPE_IS_040_PLUS(CPU_TYPE))
	{
		// CPUSH is safe to ignore - we don't emulate the cache
		return;
	}
	m68ki_exception_1111();
}

// All 68040 PFLUSH variants (PFLUSH, PFLUSHN, PFLUSHA, PFLUSHAN)
static void m68k_op_pflush40_32(void)
{
	if (CPU_TYPE_IS_040_PLUS(CPU_TYPE))
	{
		// 68040 PFLUSH variants - safe to ignore, we don't emulate TLB
		return;
	}
	m68ki_exception_1111();
}
...
static void m68k_op_pmmu_32(void)
{
	...
	// PMMU handler for cpID=0 instructions (MC68851 style)
	// This only handles PMMU instructions when PMMU is present
	// Otherwise raises F-line exception like real hardware
	{	extern int g_debug;
	if (g_debug) {
		static int pmmu32_trace_count = 0;
		if (pmmu32_trace_count < 50 && REG_PPC < 0xFFF00000) {
			pmmu32_trace_count++;
			fprintf(stderr, "[PMMU32] #%d PC=%08x IR=%04x HAS_PMMU=%d CPU_TYPE=%d\n",
				pmmu32_trace_count, REG_PPC, REG_IR, HAS_PMMU, CPU_TYPE);
		}
	}
	}
	if ((CPU_TYPE_IS_EC020_PLUS(CPU_TYPE)) && (HAS_PMMU))
	{
		// TT register access (case 1 in modes word) is handled inside m68881_mmu_ops()
		// which triggers F-line exception for MC68851 (TT regs are MC68030 only)
		m68881_mmu_ops();
	}
	else
	{
...
// cpID=7 handler - NOP these instructions
// Unix kernels use cpID=7 F-line instructions as software FPU traps
// Without hardware FPU, we just NOP them to let the kernel continue
static void __attribute__((unused)) m68k_op_cpid7_32(void)
{
	static int cpid7_count = 0;
	cpid7_count++;

	// Log first few occurrences for debugging
	{	extern int g_debug;
		if (g_debug && cpid7_count <= 10) {
			fprintf(stderr, "[cpID7-NOP] #%d PC=%08x IR=%04x - NOPing software FPU trap\n",
				cpid7_count, REG_PPC, REG_IR);
		}
	}

	// cpID=7 instructions may have extension words - skip based on EA mode
	// Format: 1111 cpID xxx mmmrrr [extension word(s)]
	// EA mode is in bits 5-3, EA reg in bits 2-0
	int ea_mode = (REG_IR >> 3) & 7;
	int ea_reg = REG_IR & 7;

	// Skip extension words based on EA mode
	switch (ea_mode) {
		case 5:  // (d16,An) - 2 bytes displacement
			REG_PC += 2;
			break;
		case 6:  // (d8,An,Xn) or full extension - at least 2 bytes
			REG_PC += 2;
			break;
		case 7:  // Absolute or PC-relative modes
			switch (ea_reg) {
				case 0:  // (xxx).W - 2 bytes
					REG_PC += 2;
					break;
				case 1:  // (xxx).L - 4 bytes
					REG_PC += 4;
					break;
				case 2:  // (d16,PC) - 2 bytes
					REG_PC += 2;
					break;
				case 3:  // (d8,PC,Xn) - at least 2 bytes
					REG_PC += 2;
					break;
				case 4:  // #imm - 4 bytes for long
					REG_PC += 4;
					break;
			}
			break;
	...
	// For cpSAVE/cpRESTORE (bits 8-6 = 100/101), no additional command word
	// For cpGEN (bits 8-6 = 000), there's a command word
	if (((REG_IR >> 6) & 7) == 0) {
		// cpGEN format - skip the command/extension word
		REG_PC += 2;
	}

	// Just consume cycles and continue
	USE_CYCLES(4);
}
...
static void m68k_op_rtd_32(void)
{
	if(CPU_TYPE_IS_010_PLUS(CPU_TYPE))
	{
		...
		REG_A[7] = MASK_OUT_ABOVE_32(REG_A[7] + MAKE_INT_16(OPER_I_16()));
		{	extern int g_debug;
		if (g_debug && new_pc < 0x00001000) {
			fprintf(stderr, "[RTD-BUG] WARNING: A7 was %08x, returning to low address PC=%08x\n",
				REG_A[7], new_pc);
			fprintf(stderr, "[RTD-BUG] Stack dump [SP-0x20 to SP+0x10]:\n");
			for (int i = -0x20; i <= 0x10; i += 4) {
				fprintf(stderr, "  [%08x] = %08x%s\n", REG_A[7] + i,
					m68k_read_memory_32(REG_A[7] + i),
					(i == 0) ? " <-- SP" : "");
			}
		}
		}
...
static void m68k_op_rte_32(void)
{
	if(FLAG_S)
	{
		...
		if(CPU_TYPE_IS_000(CPU_TYPE))
		{
            ...
            fprintf(stderr, "RTE FORMAT 8: returning to PC=%08X SR=%04X SP=%08X\n",
                new_pc, new_sr, REG_A[7]);
...
rte_loop:
		...
		switch(format_word)
		{
			case 0: /* Normal */
				...
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				/* Note: MC68030 spec Section 6.4.4.2 says to clear M when both
				 * current and stacked M=1, but V/68 kernel runs with M=1
				 * permanently and expects MSP context after RTE. Clearing M
				 * would switch to ISP and break the kernel. Instead, we use
				 * set_sr_noint below to prevent throwaway accumulation. */
				/* Trace when RTE Format 0 restores a clearly bad user-mode PC */
				if (g_debug && new_pc < 0x1000 && FLAG_S && !(new_sr & 0x2000)) {
					uint32 frame_start = REG_A[7] - 8; /* SP before the 3 pulls */
					fprintf(stderr, "[RTE-BAD-PC] Format 0 RTE restoring PC=0x%08X SR=0x%04X SP_before=%08X SP_after=%08X\n",
						new_pc, new_sr, frame_start, REG_A[7]);
					fprintf(stderr, "[RTE-BAD-PC] ISP=%08X MSP=%08X USP=%08X M=%d\n",
						m68k_get_reg(NULL, M68K_REG_ISP),
						m68k_get_reg(NULL, M68K_REG_MSP),
						m68k_get_reg(NULL, M68K_REG_USP),
						FLAG_M ? 1 : 0);
				}
				/* Syscall return tracing: log D0 and carry when returning S→U */
				if (FLAG_S && !(new_sr & 0x2000)) {
					extern int g_trace_syscalls;
					extern int g_syscall_return_pending;
					extern int g_syscall_return_num;
					extern int g_syscall_return_count;
					extern int g_kernel_pmmu_active;
					if (g_trace_syscalls && g_syscall_return_pending && g_kernel_pmmu_active) {
						extern int g_trace_copyout;
						int carry = new_sr & 0x0001;
						uint d0 = REG_D[0];
						uint d1 = REG_D[1];
						fprintf(stderr, "[SYSRET %03d] #%d D0=%08X D1=%08X carry=%d PC=%08X\n",
							g_syscall_return_count, g_syscall_return_num,
							d0, d1, carry, new_pc);
						/* Dump struct termio for ioctl TCGETA returns */
						if (g_syscall_return_num == 54 && !carry) {
							/* Read ioctl arg from user stack at saved USP */
							/* The ioctl cmd and arg were already logged in TRAP handler.
							 * Read the returned struct termio from the arg pointer.
							 * On SVR3/68k, struct termio is 18 bytes:
							 * c_iflag(2), c_oflag(2), c_cflag(2), c_lflag(2), c_line(1), c_cc[8], pad(1) */
							uint usp = m68k_get_reg(NULL, M68K_REG_USP);
							uint ioctl_cmd = m68ki_read_32_fc(usp + 8, FUNCTION_CODE_USER_DATA);
							uint ioctl_arg = m68ki_read_32_fc(usp + 12, FUNCTION_CODE_USER_DATA);
							if (ioctl_cmd == 0x5401) { /* TCGETA */
								uint16 c_iflag = m68ki_read_16_fc(ioctl_arg, FUNCTION_CODE_USER_DATA);
								uint16 c_oflag = m68ki_read_16_fc(ioctl_arg + 2, FUNCTION_CODE_USER_DATA);
								uint16 c_cflag = m68ki_read_16_fc(ioctl_arg + 4, FUNCTION_CODE_USER_DATA);
								uint16 c_lflag = m68ki_read_16_fc(ioctl_arg + 6, FUNCTION_CODE_USER_DATA);
								uint8 c_line = m68ki_read_8_fc(ioctl_arg + 8, FUNCTION_CODE_USER_DATA);
								fprintf(stderr, "[IOCTL-TCGETA-RESULT] #%d arg=0x%08X c_iflag=0x%04X c_oflag=0x%04X c_cflag=0x%04X c_lflag=0x%04X c_line=%d\n",
									g_syscall_return_count, ioctl_arg,
									c_iflag, c_oflag, c_cflag, c_lflag, c_line);
							}
						}
						g_trace_copyout = 0; /* Clear copyout tracing on syscall return */
						g_syscall_return_pending = 0;
						/* Activate ls instruction trace after fstat returns (syscall 237) */
						if (g_syscall_return_num == 28 && g_syscall_return_count >= 237) {
							extern int g_ls_trace_active;
							extern int g_ls_trace_count;
							g_ls_trace_active = 1;
							g_ls_trace_count = 0;
							fprintf(stderr, "[LS-TRACE] Activated after fstat return count=%d\n",
								g_syscall_return_count);
						}
					}
				}
				m68ki_jump(new_pc);
				/* Use set_sr_noint to prevent immediate interrupt re-entry
				 * during RTE. On MC68030 with M=1, check_interrupts inside
				 * set_sr would fire an interrupt that pushes a new throwaway
				 * on ISP before the previous one was consumed, causing ISP
				 * frame accumulation. Interrupts are checked at the next
				 * instruction boundary (m68ki_check_interrupts in the main loop). */
			...
			case 2: /* Trap */
				...
				CPU_RUN_MODE = RUN_MODE_NORMAL;
				return;
			case 8: /* Format 8 - MC68010 Short Bus Fault (8 bytes) or kernel-constructed frame */
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				m68ki_jump(new_pc);
				...
				CPU_RUN_MODE = RUN_MODE_NORMAL;
				return;
			case 9: /* Format 9 - Coprocessor Mid-Instruction (10 words, 20 bytes) */
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				m68ki_fake_pull_16();	/* internal register */
				m68ki_fake_pull_16();	/* internal register */
				m68ki_fake_pull_32();	/* instruction address */
				m68ki_fake_pull_16();	/* internal register */
				m68ki_fake_pull_16();	/* internal register */
				m68ki_jump(new_pc);
				m68ki_set_sr_noint(new_sr);
				...
				CPU_RUN_MODE = RUN_MODE_NORMAL;
				return;
			case 0xa: /* Format A - Short Bus Fault (MC68020) - 16 words total */
			{
				uint32 frame_sp = REG_A[7];
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				m68ki_fake_pull_16();	/* internal register (PSR) */
				m68ki_fake_pull_16();	/* special status word */
				...
				m68ki_fake_pull_16();	/* internal register */
				m68ki_fake_pull_16();	/* internal register */
				if (g_debug && new_pc < 0x1000 && !(new_sr & 0x2000)) {
					fprintf(stderr, "[RTE-BAD-PC] Format A RTE restoring PC=0x%08X SR=0x%04X frame_sp=0x%08X\n",
						new_pc, new_sr, frame_sp);
				}
				m68ki_jump(new_pc);
				m68ki_set_sr_noint(new_sr);
				CPU_INSTR_MODE = INSTRUCTION_YES;
			...
			case 0xb: /* Format B - Long Bus Fault (MC68020) - 46 words total */
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				/* Pull 43 more words (86 bytes) of internal state */
				{
					int i;
					for (i = 0; i < 21; i++) {
						m68ki_fake_pull_32();
		...
		/* Not handling other format types */
		if (g_debug)
			fprintf(stderr, "[RTE-FORMAT-ERROR] PC=%08x SP=%08x format=%x format_word_raw=%04x stack: %08x %08x %08x %08x\n",
				REG_PC, REG_A[7], format_word,
				m68ki_read_16(REG_A[7]+6),
				m68ki_read_32(REG_A[7]),
				m68ki_read_32(REG_A[7]+4),
				m68ki_read_32(REG_A[7]+8),
				m68ki_read_32(REG_A[7]+12));
		CPU_INSTR_MODE = INSTRUCTION_YES;
		CPU_RUN_MODE = RUN_MODE_NORMAL;
		m68ki_exception_format_error();
		return;
...
{
	uint old_sp = REG_A[7];
	uint ret_addr = m68ki_pull_32();
	/* Detect corrupted return from V/68 trap dispatch at 0x4398E */
	if (REG_PPC == 0x0004398E && (ret_addr < 0x4000 || (ret_addr > 0xD0000 && ret_addr < 0xFF800000))) {
		static int rts_bad = 0;
		extern int g_debug;
		if (g_debug && rts_bad < 3) {
			rts_bad++;
			extern unsigned long g_insn_count;
			extern void m68k_dump_pc_history(void);
			extern uint pmmu_translate_addr(uint addr_in);
			extern int g_pmmu_fault;
			fprintf(stderr, "\n[RTS-CORRUPT] PC=%08X SP_before=%08X SP_after=%08X ret=%08X insn=%lu\n",
				REG_PPC, old_sp, REG_A[7], ret_addr, g_insn_count);
			fprintf(stderr, "[RTS-CORRUPT] FLAG_S=%d ISP=%08X MSP=%08X USP=%08X M=%d\n",
				FLAG_S ? 1 : 0,
				m68k_get_reg(0, M68K_REG_ISP),
				m68k_get_reg(0, M68K_REG_MSP),
				m68k_get_reg(0, M68K_REG_USP),
				FLAG_M ? 1 : 0);
			fprintf(stderr, "[RTS-CORRUPT] CRP_aptr=%08X SRP_aptr=%08X TC=%08X SRE=%d\n",
				m68ki_cpu.mmu_crp_aptr, m68ki_cpu.mmu_srp_aptr,
				m68ki_cpu.mmu_tc, (m68ki_cpu.mmu_tc >> 25) & 1);
			/* PMMU-aware stack dump */
			fprintf(stderr, "[RTS-CORRUPT] Stack (via PMMU):");
			for (int i = -8; i < 24; i += 4)
				fprintf(stderr, " %08X", m68ki_read_32_fc(old_sp + i, FLAG_S | FUNCTION_CODE_USER_DATA));
			fprintf(stderr, "\n");
			/* Also show PMMU translation for SP */
			{
				extern void pmmu_set_fc_override(int fc);
				extern void pmmu_set_write_pending(int wp);
				pmmu_set_fc_override(5);  /* supervisor data */
				pmmu_set_write_pending(0);
				g_pmmu_fault = 0;
				uint phys_sp = pmmu_translate_addr(old_sp);
				int fault = g_pmmu_fault;
				g_pmmu_fault = 0;
				pmmu_set_fc_override(-1);
				fprintf(stderr, "[RTS-CORRUPT] PMMU: VA=%08X -> PA=%08X fault=%d\n",
					old_sp, phys_sp, fault);
				if (!fault && phys_sp < 0x01000000) {
					extern unsigned char *g_dram;
					fprintf(stderr, "[RTS-CORRUPT] Phys stack:");
					for (int i = -8; i < 24; i += 4) {
						uint a = phys_sp + i;
						uint v = (g_dram[a] << 24) | (g_dram[a+1] << 16) | (g_dram[a+2] << 8) | g_dram[a+3];
...
static void m68k_op_stop(void)
{
	if(FLAG_S)
	{
		uint new_sr = OPER_I_16();
		m68ki_trace_t0();			   /* auto-disable (see m68kcpu.h) */
		/* If STOP #$2700 (all interrupts masked), treat as NOP to allow boot to continue */
		/* This is needed because the ROM uses STOP #$2700 as error halts but without */
		/* hardware NMI, the emulator would hang forever */
		if ((new_sr & 0x0700) == 0x0700) {
			/* STOP with IPL=7 - just set SR and continue, don't actually stop */
			m68ki_set_sr(new_sr);
			return;
		}
...
static void m68k_op_tas_8_ai(void)
{
	...
	// Trace TAS for test 0 analysis - DISABLED
	int trace_tas = 0; // (ea >= 0x007FF800 && ea < 0x00800000);
	if (trace_tas) {
		fprintf(stderr, "[TAS-START] ea=%08x PC=%08x\n", ea, REG_PC);
	}

	pmmu_set_rmw_cycle(1);  // TAS is atomic R/M/W - treat read as write for PMMU M bit
	uint dst = m68ki_read_8(ea);

	if (trace_tas) {
		// Show PSR after read phase - this may be what the ROM checks
		fprintf(stderr, "[TAS-READ] ea=%08x dst=%02x PSR=%04x (N=%d) PC=%08x\n",
			ea, dst, m68ki_cpu.mmu_sr, (m68ki_cpu.mmu_sr >> 5) & 0x7, REG_PC);
	}
	...
	if (allow_writeback==1) {
		...
		if (trace_tas) {
			// Capture PSR BEFORE readback (readback triggers another translation that modifies PSR)
			uint16 psr_after_tas = m68ki_cpu.mmu_sr;
			// Read back to verify
			uint readback = m68ki_read_8(ea);
			// Show PSR value after TAS - this is what the ROM test may check
			// N bits (7-5) = number of table walk levels (0 = ATC hit)
			fprintf(stderr, "[TAS-WRITE] ea=%08x val=%02x readback=%02x PSR=%04x (M=%d N=%d) PC=%08x\n",
				ea, dst | 0x80, readback, psr_after_tas,
				(psr_after_tas & 0x0200) ? 1 : 0, (psr_after_tas >> 5) & 0x7, REG_PC);
		}
	}
}
...
{
	/* Syscall tracing: TRAP #0 is system call entry */
	extern int g_trace_syscalls;
	if (g_trace_syscalls && (REG_IR & 0xf) == 0) {
		uint d0 = REG_D[0];
		uint d1 = REG_D[1];
		uint a0 = REG_A[0];
		uint a1 = REG_A[1];
		fprintf(stderr, "[SYSCALL] #%d D0=%08X D1=%08X A0=%08X A1=%08X PC=%08X SR=%04X USP=%08X\n",
			d0 & 0xFF, d0, d1, a0, a1, REG_PPC, m68ki_get_sr(), REG_USP);
	}
...
/*   function                      mask    match    000  010  020  040 */
	...
	// MC68851 PMMU uses cpID=1 (opcodes 0xF2xx-0xF3xx)
	{m68k_op_pmmu_32             , 0xfe00, 0xf200, {  0,   0,   8,   8,   8}},
	// cpID=7 instructions (0xFE00-0xFFFF) fall through to default F-line handler
	// The kernel's own exception handler processes these for hardware detection
	// 68040 cache instructions - CINV (F4xx with bit 5=0)
	{m68k_op_cinv_32             , 0xff20, 0xf400, {  0,   0,   0,   0,   4}},
	// 68040 cache instructions - CPUSH (F4xx with bit 5=1)
	{m68k_op_cpush_32            , 0xff20, 0xf420, {  0,   0,   0,   0,   4}},
	// 68040 PFLUSH variants (F500-F53F: PFLUSHN, PFLUSH, PFLUSHAN, PFLUSHA)
	{m68k_op_pflush40_32         , 0xffc0, 0xf500, {  0,   0,   0,   0,   4}},
	// 68040 F5xx cache/MMU operations - catch all remaining F5xx as NOP
	{m68k_op_cpush_32            , 0xff00, 0xf500, {  0,   0,   0,   0,   4}},
